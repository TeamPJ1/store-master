...
export class ProductsService {
  ...
  getAllProducts(): Observable<Product[]> {
    return this.http.get<Product[]>(this.serviceUrl + "/products")
  }
  ...
  selectProduct(product: Product): Observable<Product> {
    ...
    //return this.http.put<Product>(this.serviceUrl  + "/products/" + product.id, {...product, selected: !product.selected}, httpOptions);//copie of product immutable car il est dans le store
    let updatedProduct = {...product};
    updatedProduct.selected = !updatedProduct.selected;
    return this.http.put<Product>(this.serviceUrl + "/products/" + product.id, updatedProduct);//copie of product immutable car il est dans le store
